...
# (label, band fraction, levels, re-anchor when price leaves the band?)
CONFIGS = [
    ("band5% x10 static  ", 0.05, 10, False),
    ("band5% x10 REANCHOR", 0.05, 10, True),
    ("band8% x15 REANCHOR", 0.08, 15, True),
    ("band3% x8  REANCHOR", 0.03, 8, True),
]


def simulate(hi, lo, c, band, levels, reanchor=False):
    anchor = c[0]
    lower, upper = anchor * (1 - band), anchor * (1 + band)
    ...
    for k in range(1, len(c)):
        ...
        for i in range(levels):
            if not holding[i] and L <= grid[i] <= upper:
                holding[i] = True
                entry[i] = grid[i]
        # re-anchor: price left the band → realize the bag at current price, recenter
        if reanchor and (c[k] > upper or c[k] < lower):
            for i in range(levels):
                if holding[i]:
                    realized += LOT * (c[k] - entry[i]) / entry[i] - LOT * FEE
                    holding[i] = False
            anchor = c[k]
            lower, upper = anchor * (1 - band), anchor * (1 + band)
            grid = [lower + (upper - lower) * i / levels for i in range(levels + 1)]
    ...
    return dict(realized=realized, unreal=unreal, net=net, roundtrips=roundtrips,
                ...
                pricechg=100 * (last - c[0]) / c[0], reanchors=reanchors)
...
for label, band, levels, reanchor in CONFIGS:
    print(f"\n--- {label}  (capital=${LOT*levels:.0f}/pair) ---")
    tot_real = tot_unreal = tot_net = 0.0
    for sym, (hi, lo, c) in data.items():
        r = simulate(hi, lo, c, band, levels, reanchor)
        tot_real += r["realized"]; tot_unreal += r["unreal"]; tot_net += r["net"]
        print(f"  {sym:9} trips={r['roundtrips']:<4} re={r['reanchors']:<3} "
              f"realized=${r['realized']:+6.2f} bag=${r['unreal']:+6.2f}(x{r['openpos']}) "
              f"NET=${r['net']:+6.2f} price{r['pricechg']:+.0f}%")
